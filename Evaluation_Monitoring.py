...
# ── MLflow setup (from lab) ───────────────────────────────────────
experiment_name = "Veritasium_RAG_Agent"
...
# ── Run with MLflow run tracking ──────────────────────────────────
test_queries = [
    "What do people think about gravity?",
    "Summarize the comments about the tennis racket effect",
    "What do people think about the moon landing?"
]

with mlflow.start_run(run_name="agent_evaluation_session"):
    for q in test_queries:
        ...
        mlflow.log_param(f"query_{test_queries.index(q)}", q)
        mlflow.log_metric(f"answer_length_{test_queries.index(q)}", len(result['final_answer']))